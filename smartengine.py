...
class SmartEngine(Engine):
    ...
    def negamax_background_id(self, board, depth,player, alpha = -1000000, beta= 1000000, window = 1):
        self.player_at_turn = player
        initial_a, initial_b, zobrist = alpha, beta, self.board.zobrist_hash(player)

        generated_moves = self.order_moves(list(self.board.generate_moves(player)),1)
        ...
        for move, i in zip(current_moves, range(len(current_moves))):
            if self.threadkill: 
                ...
                return
            value = self.move_negamax(player,move, alpha, beta, depth, zobrist,-1)
            moves[move] = value
            if value > best_value:
                best_value = value
